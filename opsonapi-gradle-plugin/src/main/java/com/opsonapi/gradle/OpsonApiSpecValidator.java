...
public final class OpsonApiSpecValidator {

  private OpsonApiSpecValidator() {}
  ...
  public static ValidationResult validate(File specFile, boolean failOnWarnings) {
    ...
    if (openAPI != null) {
      errors.addAll(OpsonApiEntitySchemaSupport.validateEntityOperations(openAPI, specFile));
    }
  ...
  public static ValidationResult validateWithWireSchemas(
    ...
    List<String> errors = new ArrayList<>();
    errors.addAll(OpsonApiEntitySchemaSupport.validateWireSchemas(specFile, wireSchemasDir, patchedOpenAPI));
  ...
  private static boolean isResolvableError(String message) {
